...
class BulkEditActionTest extends TestCase
{
    ...
    public function test_returns_all_eligible_fields_when_fields_array_is_empty(): void
    {
        ...
        // Aspettati: dynamic_readonly, phone, email, global (4 campi)
        // Esclusi: id, hidden_on_update, static_readonly, ecTracks (BelongsToMany), taxonomy (MorphToMany)
        // Esclusi da $exclude default: name, description
        // description viene da NovaTabTranslatable via $originalFields ma è in $exclude default
    ...
    public function test_excludes_description_by_default(): void
    {
        $action = new BulkEditAction(BulkEditTestResource::class);
        $fields = $action->fields($this->makeRequest());
        $attributes = collect($fields)->pluck('attribute')->all();

        $this->assertNotContains('description', $attributes);
    }

    public function test_includes_description_when_exclude_is_empty(): void
    {
        $action = new BulkEditAction(BulkEditTestResource::class, [], []);
        $fields = $action->fields($this->makeRequest());
        $attributes = collect($fields)->pluck('attribute')->all();
