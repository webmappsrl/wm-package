...
- Contratto: `new BulkEditAction(Resource::class, $fields = [], $exclude = ['name', 'geometry'])` — `$exclude` default protegge da bulk edit accidentale di nome e geometria; override con `[]` per disabilitare, oppure `['name']` per abilitare solo geometry
