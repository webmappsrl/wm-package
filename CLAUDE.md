...
- `canImpersonate()` compone `hasAnyRole($allowedRoles) && Gate::check('viewNova')` invece di sostituire il check nativo — difesa in profondità se `impersonation.allowed_roles` fosse misconfigurato
- `canBeImpersonated()` è `false` per il ruolo Administrator (nessun Administrator impersonabile, nemmeno da un altro Administrator) e per chi non ha il permesso `access-nova` (es. Guest) — vedi bug bloccante trovato in review qui sotto
- `impersonation.allowed_roles` configurabile (env `WM_IMPERSONATION_ALLOWED_ROLES`) invece di hardcodato — osm2cai2 ha già un proprio override locale (`App\Models\User`, non impattato), camminiditalia ha `Impersonatable` senza restrizioni (non impattato, il proprio `use Impersonatable;` diretto prevale) — vedi notes.md per il follow-up su questi due
...
| Aggiungere impersonate su Nova | oc:8231 | `src/Models/User.php`, `config/wm-package.php`, `src/Listeners/{LogImpersonationStarted,LogImpersonationStopped}.php`, `src/Listeners/EnforceNovaAccessOnLogin.php`, `src/Providers/EventServiceProvider.php`, `src/Nova/AbstractUserResource.php`, `tests/Unit/ImpersonationAuthorizationTest.php`, `tests/Feature/ImpersonationHttpTest.php`, `tests/Feature/Nova/AbstractUserResourceImpersonateTest.php` | Trait nativo Nova `Impersonatable`; `canImpersonate()` ristretto a `config('wm-package.impersonation.allowed_roles')` (default Administrator) composto con `viewNova`; `canBeImpersonated()` blocca gli Administrator e chi non ha `access-nova` (es. Guest) come target |
