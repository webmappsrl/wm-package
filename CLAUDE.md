...
- Non riusa l'helper `resolveImportEcPoiFromOsmAction()` già presente nello stesso file (decisione presa nel plan, mantenuta anche dopo che una review formale ha segnalato che l'helper sarebbe stato riusabile) — refactoring rimandato fuori scope, esplicitamente richiesto dal dev
- Ordine di merge vincolante con la PR gemella in Maphub: questa PR (wm-package) deve essere mergiata **prima** del bump del submodule pointer in Maphub
