...
|---|---|---|---|
| Fix permessi e visibilità Nova per ruolo Editor (scoping per-app EC/UGC) | oc:8162 | `src/Models/User.php` (+`hasUgcEnabled()`, +`ownedAppIds()`), `src/Nova/{AbstractEcResource,AbstractUgcResource,Layer}.php` (`indexQuery()` per `app_id`), `src/Policies/{EcPoiPolicy,EcTrackPolicy,LayerPolicy,MediaPolicy,TaxonomyActivityPolicy,TaxonomyPoiTypePolicy,TaxonomyThemePolicy,TaxonomyWherePolicy,TaxonomyTargetPolicy,TaxonomyWhenPolicy,UgcPoiPolicy,UgcTrackPolicy,UserPolicy}.php` | Taxonomy: solo Administrator scrive, Editor/Validator sola visualizzazione. Layer/EcPoi/EcTrack: scoping per `app_id` (non più `user_id`) per Editor e Validator, sia su Index (Nova `indexQuery()`) sia su Detail (Policy). UGC: Administrator/Validator accesso pieno a prescindere dall'app, Editor sola lettura scopata per app. Rimosso dead code ruoli inesistenti da 3 Policy. Vedi `docs/features/8162-.../notes.md` per 3 bug trovati e corretti in review formale post-implementazione |
...
- **Criterio EC cambiato da `user_id` (autore) ad `app_id` (appartenenza)**: `AbstractEcResource::indexQuery()`, `EcPoiPolicy`/`EcTrackPolicy::view()/update()/delete()` — un record importato da GeoHub o creato da un collega della stessa app non era visibile all'Editor con il vecchio criterio per-autore. Nuovo metodo `User::ownedAppIds(): Collection` (mirror di `hasDashboardShow()`/`hasClassificationShow()`) centralizza l'elenco delle App possedute (`apps.user_id`)
- **Layer incluso nello stesso scoping di EC** (stessa sezione menu, stesso bisogno): nuovo `before()` su `LayerPolicy` (mancava), `view()`/`update()` scopati per `app_id`, `create()`/`delete()` invariati (Editor negato, indipendente dall'app)
- **Trattamento Validator deliberatamente diverso tra EC e UGC**: per EC/Layer il Validator è scopato per `app_id` come l'Editor (nessun bypass); per UGC il Validator ha bypass pieno come Administrator (`UgcPoiPolicy`/`UgcTrackPolicy::before()`) — il ruolo Validator esiste specificamente per validare UGC cross-app, non per gestire contenuti EC di app che non possiede. **Rischio operativo aperto**: se in produzione un Validator non possiede alcuna App tramite `apps.user_id`, perde la visibilità EC — da confermare con il team
- **`MediaPolicy::before()`**: bypass totale (`return true` per chiunque) ristretto ad Administrator — stesso bug pattern trovato (ma non corretto, esplicitamente fuori scope) in `UgcPoiPolicy`/`UgcTrackPolicy::before()` all'inizio del ciclo, poi risolto anch'esso quando lo scoping UGC è stato esteso a Validator
- **`TaxonomyThemePolicy`/`TaxonomyWherePolicy` nuove** (non esistevano — nessuna policy = Nova default-allow): si affidano all'auto-discovery Laravel per convenzione di namespace (`Wm\WmPackage\Models\X` → `Wm\WmPackage\Policies\XPolicy`), verificato esplicitamente con `Gate::getPolicyFor()` nei test — nessuna `Gate::policy()` esplicita necessaria
- **3 bug trovati in review formale post-implementazione** (non dalle review per-task durante l'esecuzione): (1) `NovaServiceProvider::canSee()` alterato dopo l'approvazione — causa: un subagent di review con accesso di scrittura non vincolato; (2) `AbstractUgcResource::indexQuery()` copiato da `AbstractEcResource` senza l'esenzione Validator che la Policy gemella ha; (3) `Layer` (Nova resource) senza alcun `indexQuery()` — omissione nell'overview originale, la Policy bloccava Detail ma la lista Index non era scopata. Dettaglio completo in `docs/features/8162-.../notes.md`
- Granularità dei commit: non 1:1 per task del piano (14 task) — impossibile separare in commit distinti dato che più task hanno accumulato modifiche sullo stesso file prima di qualunque commit (nessun commit durante l'esecuzione, per policy di progetto). Commit raggruppati per area logica coerente
