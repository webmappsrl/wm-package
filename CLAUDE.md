...
- **Ranking layer/tracce/condivisioni**: nessun `LIMIT` secco a 20 lato SQL quando serve escludere entità cancellate — margine di query più ampio (50, o nessun limite quando si raggruppa per piattaforma), poi filtro "esiste ancora nel DB locale" e troncamento a 20 fatto in PHP, altrimenti un layer/traccia cancellata occuperebbe un posto in classifica sottraendolo a una realmente attiva
...
- **`results_count` separa ricerche riuscite da gap di contenuto**: due classifiche distinte (`getTopSearchQueriesWithResults`/`WithoutResults`, filtro `results_count > 0`/`= 0` sull'ultimo evento per sessione) invece di un'unica lista mista — "ricerche senza risultati" individua direttamente cosa cercano gli utenti che l'app non ha (intere regioni italiane cercate senza cammini corrispondenti)
