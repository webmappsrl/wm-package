...
class UserGeneratedDataController extends Controller
{
    ...
    private function _storeUgc(array $feature, ?User $user = null): void
    {
        ...
        if (! is_null($userGeneratedData)) {
            if (isset($feature['geometry'])) {
                $userGeneratedData->geometry = GeometryComputationService::make()->get2dGeometryFromGeojsonRAW(json_encode($feature['geometry']));
            }
    ...
    private function _storeUgcMedia(string $base64, ?string $appId = null, ?User $user = null, ?string $geometry = null): int
    {
        ...
        if (! is_null($geometry)) {
            $newMedia->geometry = GeometryComputationService::make()->get2dGeometryFromGeojsonRAW($geometry);
        }
    ...
    private function _getUgcModelFromType(string $type): string
    {
        switch ($type) {
            ...
            case 'track':
                ...
                break;
            case 'media':
                $model = "\Wm\WmPackage\Models\UgcMedia";
                break;
            default:
