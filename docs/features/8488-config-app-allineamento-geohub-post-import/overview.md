...
> **Correzione post-review (wm-skills:wm-review-ticket, rieseguita dopo il fix).** "Il browser posta la prima opzione" era un'ipotesi mai verificata contro il vero componente Nova 5 — letto direttamente `vendor/laravel/nova/resources/js/components/Controls/SelectControl.vue`: il valore effettivamente sottomesso (`modelValue`) cambia **solo** tramite l'evento nativo `change` (interazione reale dell'utente) o, all'`onBeforeMount`, solo se il valore è `null` — mai per un id non-null che non corrisponde a nessuna `<option>`. Il fatto che il browser *mostri visivamente* la prima opzione (comportamento HTML nativo quando nessuna `<option>` ha `selected`) non muta il valore che Vue sottomette al salvataggio: resta l'id originale non risolto. Il guard aggiunto in `buildLayerElement()` (Task 9) intercetta quindi correttamente questo scenario — non serve un `readonly` aggiuntivo. Il resto dell'analisi di questa causa (finestra di attesa, remap non idempotente) resta valido.
