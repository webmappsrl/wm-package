...
- [ ] Il costruttore accetta `string $novaResource` (FQCN della Nova Resource), `array $fields = []` (nomi degli attributi da esporre; array vuoto = tutti), `array $exclude = ['name', 'geometry']` (attributi sempre esclusi per sicurezza)
...
- [ ] I campi il cui `attribute` è presente in `$exclude` vengono sempre rimossi (applicato dopo il filtro `$fields`); default `['name', 'geometry']` per proteggere nome e geometria da bulk edit accidentale
