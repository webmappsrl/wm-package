...
Il tab "Theme" della Nova resource `App` (`src/Nova/App.php::theme_tab()`) viene esteso con 2 nuovi color picker (`secondary`, `tertiary`), tutti validati con una regex hex (`^#([A-Fa-f0-9]{3}|[A-Fa-f0-9]{6})$`, `nullable`) — stessa regola applicata anche ai 2 color picker già esistenti (`primary_color`, `default_feature_color`) per coerenza. `success`/`warning`/`danger` sono stati aggiunti e poi **rimossi nello stesso ciclo** dopo una verifica end-to-end della sessione frontend (vedi Decisioni in `notes.md`).
...
- [ ] `theme_tab()` espone color picker per `primary`, `secondary`, `tertiary`, `default_feature_color` (storage snake_case sotto `properties->theme->*`, coerente con i campi già esistenti) — `success`/`warning`/`danger` valutati e rimossi (vedi Decisioni)
...
- [ ] `AppConfigService::config_section_theme()` produce `config.json.THEME` con le chiavi camelCase esatte attese da `ITHEME`: `primary`, `secondary`, `tertiary`, `defaultFeatureColor`, `fontFamilyHeader`, `fontFamilyContent`
...
- [ ] Test Pest per `config_section_theme()` che verificano: (a) con tutte le 6 chiavi popolate, l'intero array `THEME` prodotto è asserito con un singolo `toBe()`/`toEqual()` sull'array completo — non `toHaveKey()` sparse, per intercettare un typo su qualsiasi chiave; (b) esclusione dei valori vuoti; (c) `properties->theme` assente/parziale non lancia eccezioni; (d) valore reale osservato sul DB camminiditalia (`primary_color: #ef7821`) tradotto correttamente in `THEME.primary`
...
- **Verifica visiva end-to-end — eseguita da una sessione parallela sul frontend, non in questo repo**: una sessione Claude separata, attiva su `webmapp-app` durante lo stesso ciclo, ha verificato a runtime (dev server locale, computed style su `:root`) che `primary`/`secondary`/`tertiary` arrivano correttamente sia a `--wm-color-*` sia a `--ion-color-*`, con contrasto WCAG ≥4.5 sui valori reali di camminiditalia. Ha anche trovato che `defaultFeatureColor` non è ancora consumato da `map-core` (usa una costante hardcoded) e che i due campi font non hanno font caricati lato app — entrambi gap **lato frontend**, fuori scope per questo repo, comunicati alla sessione frontend per il proprio ciclo.
