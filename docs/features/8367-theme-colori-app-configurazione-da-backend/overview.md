...
Il tab "Theme" della Nova resource `App` (`src/Nova/App.php::theme_tab()`) viene esteso con 2 nuovi color picker (`secondary`, `tertiary`), tutti validati con una regex hex (`^#([A-Fa-f0-9]{3}|[A-Fa-f0-9]{6})$`, `nullable`) — stessa regola applicata anche al color picker già esistente `primary_color` per coerenza. `success`/`warning`/`danger` sono stati aggiunti e poi **rimossi nello stesso ciclo** dopo una verifica end-to-end della sessione frontend; stessa sorte per `default_feature_color` (campo pre-esistente, non introdotto da questo ticket, rimosso a valle per lo stesso motivo — vedi Decisioni in `notes.md`).
...
- [ ] `theme_tab()` espone color picker per `primary`, `secondary`, `tertiary` (storage snake_case sotto `properties->theme->*`, coerente con i campi già esistenti) — `success`/`warning`/`danger` valutati e rimossi, `default_feature_color` rimosso a valle (vedi Decisioni)
...
- [ ] `AppConfigService::config_section_theme()` produce `config.json.THEME` con le chiavi camelCase esatte attese da `ITHEME`: `primary`, `secondary`, `tertiary`, `fontFamilyHeader`, `fontFamilyContent`
- [ ] Solo le chiavi con valore non vuoto vengono incluse in `THEME` — nessuna chiave con valore `null`/stringa vuota
- [ ] Le chiavi colore (`primary`, `secondary`, `tertiary`) vengono escluse dall'output se il valore sorgente non è un hex a 3 o 6 cifre (`^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$`, stessa tolleranza della regex Nova) — aggiunto in round 3 di review, prima un valore malformato scritto via API/tinker (bypassando la validazione Nova) sarebbe arrivato invariato al frontend. I 2 campi font restano testo libero, nessuna regex.
...
- [ ] Test Pest per `config_section_theme()` che verificano: (a) con tutte le 5 chiavi popolate, l'intero array `THEME` prodotto è asserito con un singolo `toBe()`/`toEqual()` sull'array completo — non `toHaveKey()` sparse, per intercettare un typo su qualsiasi chiave; (b) esclusione dei valori vuoti; (c) `properties->theme` assente/parziale non lancia eccezioni; (d) valore reale osservato sul DB camminiditalia (`primary_color: #ef7821`) tradotto correttamente in `THEME.primary`
- [ ] Verifica manuale (tinker) su camminiditalia che `config()['THEME']` prodotto rifletta i valori realmente impostati in Nova
- [ ] `config_section_map()` (originariamente riga 500, ora riga ~471 dopo i refactor successivi — vedi `sanitizeHexColor()`) legge `properties['theme']['primary_color']` invece della colonna DB morta `$this->app->primary_color` — test dedicato che verifica il fallback colore dei box "feature_collection" senza `fill_color`/`stroke_color` propri
...
- **Verifica visiva end-to-end — eseguita da una sessione parallela sul frontend, non in questo repo**: una sessione Claude separata, attiva su `webmapp-app` durante lo stesso ciclo, ha verificato a runtime (dev server locale, computed style su `:root`) che `primary`/`secondary`/`tertiary` arrivano correttamente sia a `--wm-color-*` sia a `--ion-color-*`, con contrasto WCAG ≥4.5 sui valori reali di camminiditalia. Ha anche trovato che `defaultFeatureColor` non era consumato da `map-core` (usa una costante hardcoda) — motivo per cui il campo è stato rimosso, vedi Decisioni — e che i due campi font non hanno font caricati lato app (gap lato frontend, fuori scope per questo repo, comunicato alla sessione frontend per il proprio ciclo).
...
- `success`/`warning`/`danger`: aggiunti a `theme_tab()` in una prima stesura, poi rimossi nello stesso ciclo dopo verifica end-to-end della sessione frontend — 0/0 consumatori CSS reali per success/warning (mai cablati a `--ion-color-*`), 4 per danger (usi semantici, non brand). I valori già presenti nel DB camminiditalia per questi ruoli (es. `success: #f5f5f5`, tinte "brand di scorta", non semantiche) sono stati lasciati come dato orfano in `properties->theme->*` — innocuo, non più letto da `THEME_KEY_MAP`. Se in futuro serve più spazio per il brand del cliente, la sessione frontend consiglia di valutare `light`/`dark` (50/63 consumatori reali) invece di questi tre ruoli semantici — richiede la stessa cura UX già segnalata per `dark`/`medium`/`light`, non affrontata in questo ciclo.
- `default_feature_color`: campo pre-esistente (non introdotto da questo ticket), rimosso a valle nello stesso ciclo — stessa motivazione di success/warning: la sessione frontend ha confermato 0 consumatori reali (`map-core` usa una costante hardcoded `DEF_LINE_COLOR`, non legge mai `THEME.defaultFeatureColor`). Il valore già presente nel DB camminiditalia (`#ef7821`) resta come dato orfano in `properties->theme->*`, non più letto da `THEME_KEY_MAP`. Su richiesta esplicita del dev, per coerenza con la rimozione di success/warning/danger (stesso sintomo: nessun effetto visibile, va rimosso allo stesso modo indipendentemente da quando il campo è stato introdotto).
