...
- `src/Models/User.php` (nuovo metodo `hasUgcEnabled()`, nuovo metodo `ownedAppIds()`, nuovo metodo `ownsApp()`)
- `src/Nova/AbstractEcResource.php` (`indexQuery()`: `user_id` → `app_id`)
- `src/Nova/AbstractUgcResource.php` (`indexQuery()` nuovo)
- `src/Nova/Layer.php` (`indexQuery()` nuovo, scoping per `app_id` — trovato mancante in review formale, vedi notes.md)
...
- `src/Policies/Concerns/AuthorizesViaBypassRoles.php` (trait nuovo, review post-merge: centralizza il pattern `before()` "bypass per ruolo" duplicato in `EcPoiPolicy`, `EcTrackPolicy`, `LayerPolicy`, `MediaPolicy`, `TaxonomyPoiTypePolicy`, `UgcPoiPolicy`, `UgcTrackPolicy`)
